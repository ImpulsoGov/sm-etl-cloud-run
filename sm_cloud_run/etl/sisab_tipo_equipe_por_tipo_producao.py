...
def extrair_relatorio(
    periodo_competencia: datetime.date
)-> pd.DataFrame():
    """
...
def baixar_e_processar_sisab_tipo_equipe(periodo_data_inicio: datetime.date):
    ...
    try:
        ...
        nome_arquivo_csv = f"sisab_tipo_equipe_por_tipo_producao_{periodo_data_inicio:%y%m}.csv"
        path_gcs = f"saude-mental/dados-publicos/sisab/tipo-equipe-por-tipo-producao/{nome_arquivo_csv}"
    ...
    logging.info(
        f"Processamento de SISAB - Tipo equipe por tipo produção finalizado para ({periodo_data_inicio:%y%m})."
    )
...
if __name__ == "__main__":
    ...
    baixar_e_processar_sisab_tipo_equipe(
