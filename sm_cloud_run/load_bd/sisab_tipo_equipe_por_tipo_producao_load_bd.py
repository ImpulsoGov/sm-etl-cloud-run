...
TIPOS_SISAB_TIPO_EQUIPE: Final[frozendict] = frozendict(
    {
        "id": "object",
...
COLUNAS_NUMERICAS: Final[list[str]] = [
    nome_coluna
    for nome_coluna, tipo_coluna in TIPOS_SISAB_TIPO_EQUIPE.items()
    if tipo_coluna.lower() == "int64" or tipo_coluna.lower() == "float64"
]

def transformar_tipos(df_extraido: pd.DataFrame) -> pd.DataFrame:
    """
    """    
    logging.info(f"Forçando tipos para colunas")

    sisab_equipe_producao_transformado = (
        df_extraido  
        # garantir tipos
        .change_type(
        ...
        .astype(TIPOS_SISAB_TIPO_EQUIPE)
    )
    return sisab_equipe_producao_transformado
...
def carregar_dados(
    ...
    try:
        ...
        path_gcs = f"saude-mental/dados-publicos/sisab/tipo-equipe-por-tipo-producao/sisab_tipo_equipe_por_tipo_producao_{periodo_data_inicio:%y%m}.csv"       
